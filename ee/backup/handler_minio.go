...
// minioHandler is used for 's3:' and 'minio:' URI schemes.
...
// Returns a new minio client, otherwise a nil client with an error.
func (h *minioHandler) setup(uri *url.URL) (*minio.Client, error) {
	if len(uri.Path) < 1 {
...
func (h *minioHandler) Create(uri *url.URL, file *object) error {
	glog.V(3).Infof("minioHandler got uri: %+v. Host: %s. Path: %s\n", uri, uri.Host, uri.Path)
	...
	glog.V(2).Infof("Sending data to object %q ...", object)
...
func (h *minioHandler) Load(uri *url.URL, fn loadFn) error {
	var objects []string
...
func (h *minioHandler) upload(mc *minio.Client, object string) error {
	start := time.Now()
...
func (h *minioHandler) Close() error {
...
func (h *minioHandler) Write(b []byte) (int, error) {
